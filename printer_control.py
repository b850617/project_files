...
#        what.destroy
    ...
    B2.pack()
    B3 = ttk.Button(tut,text = "Question",command = lambda:popupmsg("Not yet"))
    B3.pack()
...
def speed0():     # printer fan close 
    ...
    if errorCode == -1:
       ...
       # sys.exit()                
    errorCode=vrep.simxSetJointTargetVelocity(clientID,fan,0, vrep.simx_opmode_oneshot)

    errorCode2,fan2=vrep.simxGetObjectHandle(clientID,'fan2',vrep.simx_opmode_oneshot_wait)
    errorCode2=vrep.simxSetJointTargetVelocity(clientID,fan2,0, vrep.simx_opmode_oneshot)

    errorCode3,fan3=vrep.simxGetObjectHandle(clientID,'fan3',vrep.simx_opmode_oneshot_wait)              
    errorCode3=vrep.simxSetJointTargetVelocity(clientID,fan3,0, vrep.simx_opmode_oneshot)
 
def speed100():     # printer fan speed 100%
    ...
    if errorCode == -1:
        print('Can not find fan')
      #  sys.exit()                
    errorCode=vrep.simxSetJointTargetVelocity(clientID,fan,30, vrep.simx_opmode_oneshot)

    errorCode2,fan2=vrep.simxGetObjectHandle(clientID,'fan2',vrep.simx_opmode_oneshot_wait)
    errorCode2=vrep.simxSetJointTargetVelocity(clientID,fan2,-30, vrep.simx_opmode_oneshot)

    errorCode3,fan3=vrep.simxGetObjectHandle(clientID,'fan3',vrep.simx_opmode_oneshot_wait)              
    errorCode3=vrep.simxSetJointTargetVelocity(clientID,fan3,30, vrep.simx_opmode_oneshot)
...
class Seaofbt(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):  #程式執行時先預載入的參數或者動作
        ...
        tk.Tk.iconbitmap(self,default = "v_rep.ico")  # 改變左上角的圖示
        tk.Tk.wm_title(self,"V-rep Simulation")
        ...
        fanmenu.add_command(label = "0 %",command = speed0)
        fanmenu.add_command(label = "100 %",command = speed100)
...
class PageFour(tk.Frame):
    ...
    def __init__(self, parent,controller):
        ...
        button1 = ttk.Button(self,text = "Back",
                                    command = lambda: controller.show_frame(StartPage))
        button1.pack()#grid(row = 10,column = 2)
